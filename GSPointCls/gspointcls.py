...
from utils.graph_smoothing import compute_eigen_geometry, graph_smoothing_knn
...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        self.graph_smoothing_steps = getattr(args, "graph_smoothing_steps", 3)
        self.graph_smoothing_candidates = getattr(args, "graph_smoothing_candidates", self.k * 4)
        self.graph_smoothing_alpha = getattr(args, "graph_smoothing_alpha", 0.5)

        dim = args.dims[depth]
        nbr_in_dim = 7 if self.first else 6
        ...
        phi_dim = getattr(args, "eigen_dim", 32)
        self.phi = nn.Sequential(
            nn.Linear(3, 16, bias=False),
            nn.BatchNorm1d(16, momentum=args.bn_momentum),
            args.act(),
            nn.Linear(16, phi_dim, bias=False),
            nn.BatchNorm1d(phi_dim, momentum=args.bn_momentum),
            args.act()
        )
        ...
        self.nbr_proj = nn.Linear(nbr_out_dim + phi_dim, dim, bias=False)
        self.height_scale = getattr(args, "height_scale", 10)
        self.height_jitter = getattr(args, "height_jitter", 0.8)
    ...
    def forward(self, x, xyz, prev_knn, pwd):
        ...
        local_pwd = pwd[:, :self.n, :self.n]

        if self.graph_smoothing_steps > 0:
            knn = graph_smoothing_knn(
                local_pwd,
                k=self.k,
                steps=self.graph_smoothing_steps,
                max_candidates=self.graph_smoothing_candidates,
                alpha=self.graph_smoothing_alpha,
            )
        else:
            _, knn = local_pwd.topk(k=self.k, dim=-1, largest=False, sorted=False)

        eigenvalues_sorted, delta_p, p_prime = compute_eigen_geometry(xyz, knn)

        B, N, k = knn.shape
        nbr = delta_p.reshape(-1, 3)
        p_prime_feat = p_prime.reshape(-1, 3)

        if self.first:
            height = xyz[..., 1:2] / self.height_scale
            height -= height.min(dim=1, keepdim=True)[0]
            if self.training and self.height_jitter > 0:
                height += torch.empty((B, 1, 1), device=xyz.device).uniform_(-self.height_jitter, self.height_jitter)
            height_feat = get_nbr_feature(height, knn)
            nbr = torch.cat([nbr, height_feat, p_prime_feat], dim=1)
        else:
            nbr = torch.cat([nbr, p_prime_feat], dim=1)

        nbr = self.nbr_embed(nbr).view(B*N, k, -1).max(dim=1)[0]

        eigen_feat = eigenvalues_sorted.view(B*N, 3)
        shape_feat = self.phi(eigen_feat)

        nbr = torch.cat([nbr, shape_feat], dim=-1)
...
class GSPointCls(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.stage = Stage(args)
        self.num_points = getattr(args, "num_points", 1024)
        self.sample_train = getattr(args, "sample_train", False)
        self.sample_eval = getattr(args, "sample_eval", True)
        self.train_sample_points = getattr(args, "train_sample_points", self.num_points)
    ...
    def forward(self, xyz):
        ...
        if sample:
            sample_points = self.train_sample_points if self.training else self.num_points
            idx = pointnet2_utils.furthest_point_sample(xyz, sample_points).long()
            if sample_points != self.num_points:
                idx = idx[:, torch.randperm(sample_points, device=xyz.device)[:self.num_points]]
