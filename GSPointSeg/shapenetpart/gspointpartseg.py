...
class Stage(nn.Module):
    ...
    def forward(self, x, xyz, prev_knn, pwd):
        ...
        if self.first:
            ...
            nbr = torch.cat([nbr, get_nbr_feature(torch.cat([x, height], dim=-1), knn), p_prime_feat], dim=1)
        else:
            nbr = torch.cat([nbr, p_prime_feat], dim=1)
        nbr = self.nbr_embed(nbr).view(B*N, k, -1).max(dim=1)[0]
        shape_feat = self.phi(eigenvalues_sorted.reshape(B*N, 3))
...
class GSPointPartSeg(nn.Module):
