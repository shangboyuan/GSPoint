...
from utils.graph_smoothing import compute_eigen_geometry, graph_smoothing_indices
...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        self.phi = nn.Sequential(
            ...
            nn.BatchNorm1d(16, momentum=cp_bn_momentum),
            args.act(),
            nn.Linear(16, phi_dim, bias=False),
            nn.BatchNorm1d(phi_dim, momentum=cp_bn_momentum),
            args.act()
        )
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list):
        ...
        if self.first:
            nbr = torch.cat([delta_p, x[knn], p_prime], dim=-1).view(-1, 10)
        else:
            nbr = torch.cat([delta_p, p_prime], dim=-1).view(-1, 6)
        ...
        shape_feat = self.phi(eigenvalues_sorted)
